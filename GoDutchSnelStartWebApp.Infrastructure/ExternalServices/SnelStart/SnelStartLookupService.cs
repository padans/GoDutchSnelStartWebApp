...
public sealed class SnelStartLookupService : ISnelStartLookupService
{
    ...
    public async Task<IReadOnlyList<SnelStartAdministratieLookupDto>> GetTenantAdministratiesAsync(
        Guid tenantId,
        CancellationToken cancellationToken = default)
    {
        _logger.LogInformation(
            "Tenantgerichte SnelStart administraties ophalen gestart. TenantId: {TenantId}.",
            tenantId);
        ...
        var baseUrl = NormalizeApiBaseUrl(connection.ApiBaseUrl);
        var endpoint = $"{baseUrl}/administraties?$top=50";

        var result = await GetLookupItemsAsync(
            endpoint,
            accessToken,
            subscriptionKey,
            item => new SnelStartAdministratieLookupDto
            {
                Id = TryGetGuid(item, "id") ?? Guid.Empty,
                Naam = TryGetString(item, "naam")
            },
            "administraties",
            tenantId,
            cancellationToken);
        ...
        _logger.LogInformation(
            "SnelStart administraties ophalen afgerond. TenantId: {TenantId}, Count: {Count}.",
            tenantId,
            ordered.Count);
